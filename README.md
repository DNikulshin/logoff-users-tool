...
![Скриншот приложения](https://i.ibb.co/6y1v1S8/screenshot.png)
...
- **Сохранение настроек**:
    - Приложение автоматически сохраняет последние использованные параметры (сервер, таймер, интервал, сообщение) и загружает их при следующем запуске.
...
Приложение создает файл `settings.json` в той же папке, где находится исполняемый файл. Этот файл содержит две основные секции:
...
Проект содержит набор модульных тестов для проверки основной бизнес-логики. Чтобы запустить тесты, выполните следующую команду в корневой папке проекта:
...
Собранный файл будет находиться в папке `bin/Release/netX.X/win-x64/publish/`.
